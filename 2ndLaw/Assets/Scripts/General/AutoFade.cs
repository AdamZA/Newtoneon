...
public class AutoFade : MonoBehaviour
{
    ...
    {
        if (!m_Instance.gameObject.GetComponent<Image>())
        {
            m_Instance.gameObject.AddComponent<Image>();
        }
        m_Instance.gameObject.transform.localScale = Vector3.zero;
        Texture2D tex2d = new Texture2D(1, 1);
        tex2d.SetPixels(new Color[1] { Color.white });
        tex2d.Apply();
        m_Instance.gameObject.GetComponent<Image>().sprite = Sprite.Create(tex2d, new Rect(0, 0, tex2d.width, tex2d.height), new Vector2(0.5f, 0.5f));

        RectTransform rectTransform = m_Instance.GetComponent<RectTransform>();
        rectTransform.sizeDelta = new Vector2(Screen.width, Screen.height);
        m_Instance.GetComponent<Image>().color = new Color(aColor.r, aColor.g, aColor.b, aAlpha);
    }
